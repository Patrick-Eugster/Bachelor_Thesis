#!/bin/bash -l
#SBATCH -J fip_absgrad_resub
#SBATCH --gpus=rtx_4090:1
#SBATCH --mem-per-cpu=8G
#SBATCH --cpus-per-task=6
#SBATCH --time=23:00:00         # ONE job, loops 3 plots sequentially. ~3h45/plot × 3 ≈ 11-12h; 23h gives huge margin (gpuhe.24h cap is 24h).
#SBATCH --output=/cluster/project/cropsci/peugste/wheat3dgs/slurm_logs/fip_absgrad_resub_%j.out
#SBATCH --error=/cluster/project/cropsci/peugste/wheat3dgs/slurm_logs/fip_absgrad_resub_%j.err
#SBATCH --mail-type=END
#SBATCH --mail-user=peugste@ethz.ch

# ── AbsGrad arm — RESUBMIT of the 3 plots that did NOT finish in job 2919924 ──
# Original 48h batch (fip_test_absgrad_job.sh) died NOT from absgrad (densification was
# healthy: ~1.0-1.45M gaussians, PSNR 28.6) but from an Euler MPS failure on the node
# (CUDA Error 805). One CUDA-init call on plot_463's colored-PLY export BLOCKED for 21h
# before failing, eating the wall budget; plot_464 train then also blocked on the same
# dead MPS daemon. Plots 461/462/463/465 are done. This script redoes ONLY 464/466/467.
#
# SINGLE job, sequential loop over the 3 plots (NO job array — this account can't run array
...
# step (like the 21h export hang) eating the whole job's wall budget. Params/loggers/report
# otherwise identical to fip_test_absgrad_job.sh.
#
...
PLOT_TIMEOUT=6h     # max wall per plot. Healthy plot ~3h45 → 6h won't kill a good run; a hang dies at 6h and the loop moves on. 3×6h=18h < 23h job cap.
PLOTS=(plot_464 plot_466 plot_467)

# One combined run-report for all 3 plots (run_reconstruction.py APPENDS each plot's report here).
...
# ════════════════════════════════════════════════════════════════════
#  LOOP over the 3 unfinished plots — full pipeline + iter-eval each
# ════════════════════════════════════════════════════════════════════
...
# ── Peak usage summary (whole job, across all 3 plots) ──
echo ""
echo "========================================"
...
# ── Combined run report (all 3 plots: per-step status + times + error tails) ──
echo ""
echo "========================================"
