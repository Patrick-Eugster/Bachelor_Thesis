#!/bin/bash -l
#SBATCH -J seg_A0715_ocv15k_perhead_sam1
...
#SBATCH --output=/cluster/project/cropsci/peugste/wheat3dgs/slurm_logs/seg_A0715_ocv15k_perhead_sam1_%j.out
#SBATCH --error=/cluster/project/cropsci/peugste/wheat3dgs/slurm_logs/seg_A0715_ocv15k_perhead_sam1_%j.err
#SBATCH --mail-type=END,FAIL
#SBATCH --mail-user=peugste@ethz.ch

# ============================================================================
# A/0715 seg — PER_HEAD SAM1 @ conf 0.35 (opencv, full cull). SEG ONLY: reuses the per_head SAM1 masks
# pre-baked by scripts/phone_A0715_perhead_maskgen_job.sh (MASK_EXP perhead_sam1_conf035) so no mask-gen
# runs inside this 24h slot. Everything else is IDENTICAL to the per_tile control ocv15k_frust_paint
# (opencv baseline model, full cull, conf 0.35, SAM1) EXCEPT sam_crop_mode=per_head. So the A/B is:
#   ocv15k_perhead_sam1 (per_head) vs ocv15k_frust_paint (per_tile, 0.369) -> granularity effect at SAM1, conf 0.35.
# Pairs with the SAM2 per_head run (ocv15k_perhead_sam2) for SAM1-vs-SAM2 at per_head.
#
...
ITER=15000
MASK_EXP=perhead_sam1_conf035            # per_head SAM1 masks pre-baked by the Queue-B mask-gen job
SEG_EXP=ocv15k_perhead_sam1              # NEW seg output name
...
[ -n "$(ls -A "$MASK_OUT/masks" 2>/dev/null)" ] || { echo "  X per_head SAM1 masks missing (run the Queue-B mask-gen job first): $MASK_OUT/masks"; FAIL=1; }
...
echo "  ok: opencv baseline model + images + markers + per_head SAM1 masks present, seg target fresh"
...
# the masks (per_head SAM1 here vs per_tile SAM1 there).
