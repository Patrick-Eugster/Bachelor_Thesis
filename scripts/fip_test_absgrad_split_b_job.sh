#!/bin/bash -l
#SBATCH -J fip_absgrad_B
#SBATCH --gpus=rtx_4090:1
#SBATCH --mem-per-cpu=5G          # 5G × 6 = 30G total — fits a 32 GB node with headroom.
#SBATCH --cpus-per-task=6
#SBATCH --time=24:00:00           # 4 plots × ~4.8h ≈ 19-20h; 24h cap. PLOT_TIMEOUT 6h × 4 = 24h hard cap.
#SBATCH --output=/cluster/project/cropsci/peugste/wheat3dgs/slurm_logs/fip_absgrad_B_%j.out
#SBATCH --error=/cluster/project/cropsci/peugste/wheat3dgs/slurm_logs/fip_absgrad_B_%j.err
#SBATCH --mail-type=END
#SBATCH --mail-user=peugste@ethz.ch

# ── AbsGrad arm — SPLIT B: plots 464/465/466/467 ──
# Half of the full 7-plot rerun (fip_test_absgrad_all_job.sh), split so it can run on its own
# GPU node in parallel with SPLIT A (461/462/463). Each half fits under the 24h cap and on
...
# MPS/CUDA hang is killed after PLOT_TIMEOUT and the loop continues. 4 × 6h = 24h = cap.
...
PLOTS=(plot_464 plot_465 plot_466 plot_467)
...
echo ""
echo "========================================"
echo "  PEAK USAGE SUMMARY (SPLIT B)"
echo "========================================"
...
echo ""
echo "========================================"
echo "  COMBINED RUN REPORT (SPLIT B)"
echo "========================================"
