#!/bin/bash -l
#SBATCH -J phone_absgrad_opencv_Y
#SBATCH --gpus=rtx_4090:1
#SBATCH --mem-per-cpu=7G      # 42 GB total (6 × 7G). MEASURED peak = 30.0 GiB on fA/0715 (this group's
#SBATCH --cpus-per-task=6     #   heaviest, 96 imgs) in phone_absgrad_20250715 -> 42 GB gives ~12 GiB headroom.
#SBATCH --time=03:59:00       # TIME: MEASURED 46m/session = 32m train + 14m RENDER + 0.5m metrics (render
#                             #   ~100 full-res views @3M Gaussians); 4 sessions (all <=96 imgs) ~3h, ~1h margin.
# ⚠️ VRAM: that run peaked 22.83/24 GiB — densify_grad_threshold=0.0008 BARELY fits the 4090. If any
#    train_vanilla crashes with CUDA OOM, raise the threshold to 0.001.
#SBATCH --output=/cluster/project/cropsci/peugste/wheat3dgs/slurm_logs/phone_absgrad_opencv_Y_%j.out
#SBATCH --error=/cluster/project/cropsci/peugste/wheat3dgs/slurm_logs/phone_absgrad_opencv_Y_%j.err
#SBATCH --mail-type=END
#SBATCH --mail-user=peugste@ethz.ch

# ============================================================================
# PHONE RECON — AbsGS on the OPENCV arm, GROUP Y (the other 4 of 8 sessions).
# ----------------------------------------------------------------------------
...
EXP=absgrad
# ordered by image count ASCENDING (75,88,93,96) so the highest-VRAM session (fA/0715, 96 imgs, the one
# that peaked 22.83 GiB in the past AbsGS run) runs LAST — a crash there doesn't waste the safe ones.
SESSIONS=("field_D 20250706" "field_D 20250627" "field_A 20250618" "field_A 20250715")
...
# ── summary ──
echo ""
echo "================ opencv+AbsGS (group Y) PER-SESSION STATUS ================"
for s in "${SESSIONS[@]}"; do
  set -- $s; FIELD=$1; DATE=$2
