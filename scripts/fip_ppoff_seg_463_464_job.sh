#!/bin/bash -l
#SBATCH -J fip_ppoff_AB
#SBATCH --gpus=rtx_4090:1
#SBATCH --mem-per-cpu=4G       # 32 GB total — FIP seg RAM peak ~17 GB (crop cache); ample. No render_360.
#SBATCH --cpus-per-task=8
#SBATCH --time=03:59:00        # <4h short-job tier. 2 plots; per plot ~ train 26m + seg (pp-off, may be slower) ~40m + eval; ~2.5h expected.
#SBATCH --output=/cluster/project/cropsci/peugste/wheat3dgs/slurm_logs/fip_ppoff_%j.out
#SBATCH --error=/cluster/project/cropsci/peugste/wheat3dgs/slurm_logs/fip_ppoff_%j.err
#SBATCH --mail-type=END,FAIL
#SBATCH --mail-user=peugste@ethz.ch

# ============================================================================
# FIP 3D-SEG principal-point A/B — the pp-OFF arm (the pp-ON arm already exists as fipseg15k_pp).
# For each plot it TRAINS a fresh pp-OFF model and SEGMENTS it pp-OFF (both must match — a pp-on model
# segged pp-off collapses matching). Single detector = YOLOv5 @1280 (held constant vs the pp-on arm).
# Recon: gsplat + use_principal_point=FALSE + 15k + resolution 1 + absgrad OFF. Seg opts: crop cache +
# frustum_cull (lossless); NO roi/marker/ground cull (FIP has no markers). No render_360 (train+render+metrics + seg + eval + eval2d only).
# Compare the resulting eval_2d IoU against the pp-ON fipseg15k_pp/seg_yv5_1280 to isolate the pp fix.
#
# This script does plots 463 + 464. Submit:
#     sbatch /cluster/project/cropsci/peugste/wheat3dgs/scripts/fip_ppoff_seg_463_464_job.sh
# ============================================================================
set -euo pipefail
...
for PLOT in "${PLOTS[@]}"; do
  ...
  echo ""; echo "======== [$PLOT] TRAIN (15k, gsplat, pp=FALSE) ========"
  TRC=0
  ...
  echo ""; echo "======== [$PLOT] SEG $SEGEXP (pp=FALSE) ========"
  python src/run_reconstruction.py "${RECON_COMMON[@]}" \
