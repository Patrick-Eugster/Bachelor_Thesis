#!/bin/bash -l
#SBATCH -J seg_A0715_pin15k
#SBATCH --gpus=rtx_4090:1
#SBATCH --mem-per-cpu=8G       # 64 GB total — phone 3D seg decodes many full-res masks (RAM-heavy)
#SBATCH --cpus-per-task=8
#SBATCH --time=18:00:00        # 15k model (~1.23M gaussians): maskgen ~30 min + seg ~7-8h + eval; 18h ceiling
#SBATCH --output=/cluster/project/cropsci/peugste/wheat3dgs/slurm_logs/seg_A0715_pin15k_%j.out
#SBATCH --error=/cluster/project/cropsci/peugste/wheat3dgs/slurm_logs/seg_A0715_pin15k_%j.err
#SBATCH --mail-type=END,FAIL
#SBATCH --mail-user=peugste@ethz.ch

# ============================================================================
# A/0715 seg — arm 1 of the 2x2 (SfM x training-length): PINHOLE baseline, 15k.
...
FIELD=field_A
DATE=20250715
MODEL_EXP=baseline                       # 3DGS model to segment (results/.../vanilla_3dgs/<MODEL_EXP>)
ITER=15000
TARGET=4032                              # pinhole is native 4032 -> letterbox r=1.0 (no scaling)
MASK_EXP=yolov5_pertile_pin15k           # unique per job (avoids cross-job mask-path collision)
SEG_EXP=pin15k_yolov5_pertile            # seg output name (model/segmentation_3d/<SEG_EXP>)
