...
| [PHONE_SFM_FRONTEND.md](PHONE_SFM_FRONTEND.md) | ✅ | **Closing the COLMAP-vs-Agisoft registration gap on phone wheat.** Our SIFT run didn't *fail* (110/113 registered) but **fragmented** into 4 sub-models (sparse cross-cluster matching), while Agisoft kept one. Fix: **ALIKED + LightGlue** (learned detector + matcher, native in COLMAP 4.1) → **1 model / 113-of-113, denser than Agisoft**; **sequential** matching = same result ~8× faster. Wired into `run_colmap.py` (`front_end=sift\|aliked`, SIFT kept as baseline). Includes the `analyze_sfm_connectivity.py` scorer, the full SIFT/ALIKED/Agisoft benchmark tables, the CUDA-12 onnxruntime lib shim, and the VRAM/CPU profile. |
...
| [MARKER_INTEGRATION_PLAN.md](MARKER_INTEGRATION_PLAN.md) | ✅ | **Plan + running log** for bringing the Agisoft coded markers into our COLMAP. Background: phone-vs-FIP data map, why detection is only *half* the job (§11c), the per-marker ≥2-views rule. **Pipeline now COMPLETE end-to-end** (§8): detect/decode (v8 + manifest filter) → **triangulate** (`triangulate_markers.py`, snap/seed, beats Agisoft recall) → **Step 3 metric scale** (`marker_scale.py`, ~14–18 mm vs surveyed XYZ, tape cross-check) → **Level B Flavour 1** metric model (`apply_metric_transform.py` → `sparse_metric/`) → **Level B Flavour 2** GCP-constrained BA (`marker_gcp_ba.py` via pycolmap → `sparse_metric_gcp/`). **Finding:** markers give real-world SIZE, not extra SfM accuracy (scene reproj unchanged; 6 coplanar markers anchor scale/datum only). field_A ~18 mm residual is survey-limited (open Q: RTK vs total-station). Next = feed metric model into 3DGS for metric phenotyping. |
| [MARKER_DETECTOR_LATE_SEASON.md](MARKER_DETECTOR_LATE_SEASON.md) | ✅ | **The late-season recall fix.** Our v8 CCT detector collapsed on late-season dense canopy (`field_D/20250722`: localized only 13% of Agisoft's marker pins → metric scale **blocked**, 2 markers were 600–1100 px-reproj false positives). Gate attribution vs Agisoft GT showed **NCC finds 82% of plates but the `white_surround` gate kills 57 of 75** — because it identifies the plate by *brightness* and in late season the wheat is brighter than the grey/shaded plate. Fix: a **brightness-invariant `plate_gate=lowsat`** (plate = achromatic / low HSV-saturation, no brightness test; `plate_s_max=110`). Recall **13%→79%**, false positives *eliminated* (consensus outvotes them once it has the true detections), **zero new FPs across all 14 sessions**, 0722 unblocked (6/6 markers, CV 2.24%). lowsat now default; `white` kept for A/B. Includes the why-no-new-FPs argument + the 14-session table. |
| [MARKER_COLMAP_RERUN_EXPERIMENT.md](MARKER_COLMAP_RERUN_EXPERIMENT.md) | 📝 | **PLANNED experiment** — does integrating markers *into* COLMAP (intrinsic-refined GCP-BA, or markers in the mapper) beat the current post-hoc approach (Flavour 1/2, intrinsics frozen → scene reproj unchanged)? Key design: **leave-one-marker-out** (anchor 5, predict the 6th in mm) to avoid the circularity trap, + Agisoft as independent reference. First pass = **Arm A** (refine intrinsics in GCP-BA) + LOMO, geometry metrics only (3DGS deferred), on the 3 good sessions. Decision rule + escalation to global-BA / incremental-mapper arms. Hypothesis to test: 6 coplanar markers can't improve internal geometry → Flavour 1 stays sufficient. |
| [MARKER_DETECTION_STAGE_A.md](MARKER_DETECTION_STAGE_A.md) | ✅ | **How the Stage A marker localizer works** (`detect_markers.py`, **v1**), explained for a CS reader: the 5-step pipeline — HSV color filter (bright + colorless) → morphology clean-up → contour shapes → size/squareness/**convex-hull solidity** checks → "has a black disk?" dark-fraction check → disk-centroid center. Includes the hull-vs-raw-area fix, what it gets right/wrong, the parameter table, and how the thresholds were calibrated from real plates. |
| [MARKER_DETECTION_CCT.md](MARKER_DETECTION_CCT.md) | ✅ | **Option C — coded-target detect-and-decode** (adopting [poxiao2/CCTDecode](https://github.com/poxiao2/CCTDecode)): terminology (plate/disk/white-dot/arcs), how `CCT_extract` works (Otsu → contour → ellipse-fit → **affine-rectify** → validate → ring-sample → 12-bit decode → ID), and the full build: Phase 0 (Agisoft spec-PDF marker decodes → approach viable), phone **GT** arrived (`marker_projections.csv`, asymmetric — high precision/low recall), Phase 0b/1 (stock blob-search reads **arcs** → "7" artifact; **forcing the decode onto the disk** → all 6 markers distinct consistent IDs), **v7** (`detect_markers_v7_cct.py`: v6 proposes → forced-center decode; **fill-ratio** = size-independent disk-vs-arc discriminator; disk-center reported → 0.7 px; bypass `CCT_or_not`; **73% recall of Agisoft**), **v8** (concentric-consensus + **re-centering**: recovers center from arcs when v6 lands on an arc / disk occluded; **76% recall**, fixes the 113↔7 arc bug). Tools: `compare_v7_vs_agisoft.py` (recall + ID-map + miss CSVs), `overlay_agisoft_markers.py`, `debug_cct_window.py`. ID map 113↔T1…77↔T6. Next: min-views cleanup / triangulation → GCPs (Option D). |
| [MARKER_CODE_STRUCTURE.md](MARKER_CODE_STRUCTURE.md) | ✅ | **12-bit code structure / coding theory** — how the ID is built (12 ring bits ↔ arcs), **`B2I` rotation-canonicalization** (min integer over 12 rotations) and rotation-aware Hamming, the **legal set = 352 of 4096** rotation-canonical necklaces — but **NOT a junk filter** (decoded codes are always canonical → always legal; the all-ones junk `511/1023/…` are legal necklaces). **KEY findings: (a)** legal ≠ separated (min Hamming over all legal codes = 1, 2004 distance-1 pairs); **(b)** the 6 deployed codes `{77,85,89,101,105,113}` are **ground truth decoded from the spec PDF** (6 pages → target1=113 … target5=**85** … target6=77), confirming **117 = 1-bit misread of 85** (target 5). Real junk filter = **plot manifest** (= v8 `id_filter=manifest`, the PDF codes), then **triangulation-by-location** as final arbiter (snaps 117→85; majority vote / Hamming alone unsafe). **TODO:** future coded-target **generator tool** (pick K codes with max-min Hamming ≥3 → error-correction provably safe). |
| [MARKER_DETECTION_VERSIONS.md](MARKER_DETECTION_VERSIONS.md) | ✅ | **All eight detector versions (v1→v8) compared** — cue/theory, algorithm, result for each: v1 (white square), v2 (ellipses + size-aware clustering), v3 (fiducial disk), v4 (**hybrid**), v5 (synthetic-template NCC), v6 (**real cropped fiducial + multi-scale NCC + fiducial-snap + ellipse-fit + dedup — best recognition-based version**), v7 (**CCT decode**: v6 centers → 12-bit decode, fill-ratio disk finder), v8 (**CCT decode + concentric re-centering**, `id_filter=manifest` — **production, used by whole pipeline**). The failure driving each next version, the side-by-side table, and the **two ceilings** (heuristics, then template matching) → the decisive switch to **Option C (CCT decode), not the once-planned Option B (CNN/YOLO)**: decoding the self-validating codeword kills false positives and yields the marker ID for free. Deep dive on v7/v8 in [MARKER_DETECTION_CCT.md](MARKER_DETECTION_CCT.md). |
