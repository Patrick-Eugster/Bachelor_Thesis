...
| [MARKER_INTEGRATION_PLAN.md](MARKER_INTEGRATION_PLAN.md) | ✅ | **Plan** for bringing the Agisoft coded markers into our COLMAP — both the post-hoc distance-accuracy check **and the real goal: markers IN the SfM** (tie-points + GCP scale, §11b). The phone-vs-FIP data map, why detection is only *half* the job, how false positives actually matter (§11c), the per-marker ≥2-views rule, and the current detector status (heuristics **and** template matching both hit a ceiling → going learned, **Option B** CNN/YOLO). |
...
| [MARKER_DETECTION_VERSIONS.md](MARKER_DETECTION_VERSIONS.md) | ✅ | **The six detector versions (v1→v6) compared** — cue/theory, algorithm, result for each: v1 (white square), v2 (ellipses + size-aware clustering), v3 (fiducial disk), v4 (**hybrid**), v5 (synthetic-template NCC), v6 (**real cropped fiducial + multi-scale NCC + fiducial-snap + ellipse-fit + dedup — best version**). The failure driving each next version, the side-by-side table, and the **two ceilings** (heuristics, then template matching) → the decision to go learned: **Option B**, a trained CNN/YOLO marker detector. v6 is best but still ~1/5 correct on cluttered frames. |
