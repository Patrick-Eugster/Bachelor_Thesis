...
| [MARKER_DETECTION_CCT.md](MARKER_DETECTION_CCT.md) | ✅ | **Option C — coded-target detect-and-decode** (adopting [poxiao2/CCTDecode](https://github.com/poxiao2/CCTDecode)): how `CCT_extract` really works (global-Otsu → contour → ellipse-fit → **affine-rectify** → `CCT_or_not` validate → ring-sample → 12-bit decode → ID + center), why decoding kills false positives + gives IDs for free, the two mismatches with our data (global Otsu on a 4000 px field frame, the 2.5× ring-radius assumption), Otsu-vs-adaptive thresholding, the keep-the-core/replace-the-front-end scope, the marker-constellation prior, and the phased plan. |
