...
| [MARKER_INTEGRATION_PLAN.md](MARKER_INTEGRATION_PLAN.md) | ✅ | **Plan** for bringing the Agisoft coded markers into our COLMAP — both the post-hoc distance-accuracy check **and the real goal: markers IN the SfM** (tie-points + GCP scale, §11b). The phone-vs-FIP data map, why detection is only *half* the job, how false positives actually matter (§11c), the per-marker ≥2-views rule, and the current detector status (heuristics hit their ceiling → template-match → CNN). |
...
| [MARKER_DETECTION_VERSIONS.md](MARKER_DETECTION_VERSIONS.md) | ✅ | **The four detector versions (v1→v4) compared** — cue/theory, algorithm, and result for each: v1 (white square), v2 (ellipses of the rings + size-aware clustering), v3 (the central fiducial disk + white dot), v4 (**hybrid**: v2 finds regions, v3 snaps the center locally + contrast-relative/local-Otsu thresholds). The failure that drove each next version, the side-by-side table, the **heuristic ceiling**, and the next plan — **Option A** template-match the bullseye (the simplest ancestor of a CNN) → **Option B** a trained CNN/YOLO detector. |
