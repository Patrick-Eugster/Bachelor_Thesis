...
"""
import sys
# NOTE (vendored edit): made the `progress` import lazy. Upstream imported it at
# module top, but it's only used by the batch progress bar in scanne_data_dir().
# Importing it here would force the `progress` dependency just to use the decode
# core. Moved the import inside scanne_data_dir() so the core is dependency-light.
import argparse
import numpy as np
# NOTE (vendored edit): removed `from numpy import *`. On numpy 2.x / Python 3.12 it
# shadowed the Python builtins max/min/round/sum with numpy versions, which broke the
# code (e.g. `max(scalarA, scalarB)` became `np.max(a, axis=b)` -> "'float' object
# cannot be interpreted as an integer"). The module already accesses numpy via `np.`
# and math via `math.`, so the star import was unused and only caused harm.
import cv2
import os
...
def scanne_data_dir(src_folder,save_folder,N,R,color):
    from progress.bar import ShadyBar  # lazy import (see top-of-file note)
