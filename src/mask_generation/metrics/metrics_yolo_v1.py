    ...
    for plot_name in sorted(os.listdir(input_dir)):
        ...
        for fname in sorted(os.listdir(label_dir)):
            if fname.endswith('.txt'):
                ...
                labeled.append((input_plot_dir, result_plot_dir, os.path.join(label_dir, fname), stem))
    return labeled
...
def evaluate_all_plots(iou_threshold=None):
    ...
    for idx, r in enumerate(per_plot_results):
        bboxes_with_conf_path = os.path.join(r['_result_plot_dir'], 'bboxes_with_conf', r['image_stem'] + '.pt')
        ...
        all_gt_boxes_for_ap.append(load_gt_boxes(
            os.path.join(r['_input_plot_dir'], 'manual_label', r['image_stem'] + '.txt'),
            r['img_w'], r['img_h']))
