...
prompts**. Browser-based, zero new deps (stdlib HTTP server).

## Run

```bash
# base env: torch + ultralytics
...
> (`Ctrl+Shift+R`). The page is served fresh each load, and `server.py` is loaded once at startup.

## Choosing which images to label

The tool does not scan a folder for images. It shows the frames listed in
`input_plots/phone/gt_selection.json`, which maps each session to the frames to label:

```json
{
  "field_A/20250715": [
    { "stem": "IMG_20250715_153912" },
    { "stem": "IMG_20250715_154001" }
  ]
}
```

Only `stem` matters. The tool resolves each one to
`input_plots/phone/<field>/<date>/images/<stem>.png` (or `.jpg`). To label your own
images, edit that file and set the sessions and frames you want. The shipped file holds
the selection used for the thesis.

## Workflow

1. Pick a GT image in the list. With **`⚙ auto-seed` OFF (default)** it opens **blank**. With it ON (or
...
3. **Fix seeds** (if seeded): select a loose mask → `E` → a couple points → Run. `Del` removes false ones,
   and `L` locks the good ones.
4. **💾 Save** → writes into `input_plots/phone/<field>/<date>/manual_label/` (`<stem>_gt_mask.png` = union
   of the **active set** = the eval GT, plus `<stem>_sets/` with every set, and `<stem>_meta.json`).
...
- **`set:` dropdown + `＋`** (top bar) — keep multiple mask-sets per image, switch between them, or add a new
...
- **`🗑 clear`** (click to arm → click to confirm) — moves the set's masks into a **`⟲ set N` backup set**
  (kept in the dropdown, and locked masks stay).
...
- **SAM2.1-large**, per-head crop (the resolution lever). First seed of an image ~48 s (cached, so reopening is fast).
  Each point-click is sub-second.
- Single active image / single user. SAM calls are serialized on one GPU (~6 GB interactive).
