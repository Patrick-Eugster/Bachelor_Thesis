...
from wheatheadsmorphology.bbox_functions import (
...
def run_pipeline(cfg: dict) -> None:
    ...
    for file_i, file_path in enumerate(
        tqdm(file_paths, desc="Processing wheat heads", total=len(file_paths))
    ):
        ...
        # TODO: support user specified global shift (e.g. plot centroids in Swiss CRS)
        ...
        if data.shape[0] > sor_parameters["k"]:
            data, _ = statistical_outlier_removal(
                data, k=sor_parameters["k"], std_ratio=sor_parameters["std_ratio"]
            )
        ...
        results_df.loc[file_i, ["X", "Y", "Z"]] = (
            np.mean(data[:, :3], axis=0) + global_shift
        )
        # Save number of points
        results_df.loc[file_i, ["ptnr"]] = data.shape[0]
        # Estimate Length, Width, Volume, Inclination, Curvature
        results_df.loc[
            file_i, ["length", "width", "volume", "inclination", "curvature"]
    ...
    if get_bboxes is True:
        ...
        file_path_bboxes = output_folder / filename_bboxes
    ...
    if get_processed_pcd is True:
        ...
        output_checking_preprocessing = output_folder / pcd_output_name
