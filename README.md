...
each head a consistent ID across all views. The original pipeline was built for an
...
(`src/run_reconstruction.py`). It can run train → render → metrics → 3D
segmentation → export → evaluation, but every step is **off by default**, so you
switch on the ones you want with `run_*` flags (for example `run_train=true`).
...
Each `src/` stage folder has its own `README.md` with the full list of options —
start there when you want to change how a stage behaves.
...
For FIP the data comes ready to use and skips SfM. For phone, only `input/` is
provided and preprocessing generates the rest.
...
│   │
│   └── phone/<field>/<session>/<method>/<exp>/
│       ├── bboxes/
│       └── masks/
│
└── reconstruction/
    ├── fip/<plot>/vanilla_3dgs/<exp>/
    ...
    │
    └── phone/<field>/<session>/vanilla_3dgs/<exp>/
        ├── point_cloud/
        ├── train/  test/
        ├── results.json
        └── segmentation_3d/<exp>/
...
visualizations, the saved config, and logs.

## Quickstart

The pipeline supports two datasets, **phone** and **fip**, and each one has its own
...
the right dataset, defaults, and per-stage values for you.

Every stage writes into a folder named by `experiment_name`. The default name is
`thesis_baseline`, which is shared across mask generation, reconstruction, and
segmentation, so the stages find each other's outputs and chain automatically.
...
### Stages 3 + 4 — Reconstruction and 3D segmentation
...
# or one at a time — segmentation reuses the already-trained model
...
# 3+4. reconstruction and 3D segmentation — here plot is the field, date the session
...
- **Phone mask generation uses a marker-based ROI tuned to our field.** When
  `roi.enabled` is on, it restricts detection to our region of interest. This region
  is built from the triangulated ground markers, so it needs the preprocessing marker
  steps (`run_markers=true`) to have run first. On data without those markers, mask
  generation still runs without error. The ROI simply falls back to covering the whole
  image. You can also force it off with `roi.enabled=false`.
